...
function error-exit {
  ...
  echo "looks like something went wrong. why: $?"
...
echo "
Format: -
sysconfig.sh -h | --help    (help)
sysconfig.sh --host         (hostname)
sysconfig.sh --os           (os information)
sysconfig.sh --domain       (domain information)
sysconfig.sh --ipconfig     (ip information)
sysconfig.sh --cpu          (cpu information)
sysconfig.sh --memory       (ram information)
sysconfig.sh --disk         (disk information)
sysconfig.sh --printer      (printer information)"
...
function cleanup {
  ...
  rm /tmp/*.$$
  [ $? -eq 0 ] && echo "Temporary files of this script deleted..."
  exit

}

# Trapping cleanup function to run before exit
trap cleanup SIGINT
